              ...
              if (!t.metaKey && !t.ctrlKey && !t.shiftKey && !t.altKey) {
                if (t.button === 0) {
                  ...
                  if (
                    ...
                    if (n.getHash() != "") {
                        // Get the hash value from n.getHash() and sanitize it
    let hash = n.getHash();
    console.log("Original hash:", hash);

    // Ensure the hash starts with '#' and contains valid characters (letters, numbers, underscores, and dashes)
    if (hash.startsWith('#')) {
        hash = hash.replace(/[^a-zA-Z0-9_-]/g, '');  // Remove any invalid characters
    } else {
        // If the hash doesn't start with '#', reset it to an empty string or handle accordingly
        hash = '';
    }
    if (hash && document.querySelector(`#${hash}`)) {
                      this.triggerEvent("samePageWithHash", t);
                    //   console.log(n.getHash());
                      var r = document.querySelector(`#${hash}`);
                      if (r != null) {
                        history.replaceState(
                          {
                            url: n.getAddress() + `#${hash}`,
                            random: Math.random(),
                            source: "swup",
                          },
                          document.title,
                        ...
                        console.warn(
                          "Element for offset not found (" + `#${hash}` + ")"
                        );
                      }
                    } 
                    else {
                        console.warn("Invalid or empty hash:", hash);
                    }
                }else {
